    ...
    <form class="form" method="post" action="#">
      ...
      <div class="form__content">
        ...
        <div class="styled-input">
          <input type="text" class="styled-input__input" name="nickname" placeholder="Username" v-model="formMess.nickname">
<!--          <div class="styled-input__placeholder"><span class="styled-input__placeholder-text">Username</span></div>-->
          <div class="styled-input__circle"></div>
        </div>
        <div class="styled-input">
          <input type="password" class="styled-input__input" placeholder="Password" v-model="formMess.password" maxlength="16">
<!--          <div class="styled-input__placeholder"><span class="styled-input__placeholder-text">Password</span></div>-->
          <div class="styled-input__circle"></div>
        </div>
        <button type="button" class="styled-button" @click.prevent="submit"><span class="styled-button__real-text-holder"> <span
            class="styled-button__real-text">Submit</span> <span class="styled-button__moving-block face"> <span
            class="styled-button__text-holder"> <span class="styled-button__text">Submit</span> </span> </span><span
...
<script>
  export default {
    ...
    methods: {
      // 点击登录
      submit(e){
        if(this.formMess.nickname.trim().length<=0||this.formMess.nickname==''){
          this.$message.error('请输入用户名')
          return;
        }
        if(this.formMess.password.trim().length<=0||this.formMess.password==''){
        ...
        let url = this.query.url||'/home'
        this.$local.set('userInfo',data,1000*60*60*24)
        this.$router.push({path:url,query:1})
      }
    },
    created(){
    ...
    mounted() {
      var that = this;
      that.inputs = document.querySelectorAll('.styled-input__input');
