    ...
    <div class="padding-xs padding-top-sm padding-bottom-sm sidebar-item" v-for="(items,index) in sideData"
      ...
      <div class="flex sidebar-item-son">
        <div class="flex-sub text-cut padding-lr-xs text-center" v-for="(item,idx) in items.fields[0].fields"
            ...
            @click="getSubData(item.name,item.id,sideData[0].fields[0].name,index,items.fields[0].url,item.url)">{{item.name}}</span>
        </div>
      </div>
          ...
          <div class="text-center padding-lr-xs margin-bottom" style="width: calc(100%/6);box-sizing: border-box" v-for="(subitem,idx) in items.fields" :key="idx">
            ...
            <div class=" base sub-menu-list">
              <div class="margin-bottom-sm sub-menu-item text-center pointer" :class="curItem==index&&curOpt==lowItem.id?'text-cyan':''"
                   ...
                   :key="subidx" @click="getSubData(lowItem.name,lowItem.id,subitem.name,index,subitem.url,lowItem.url)">
                {{lowItem.name}}
              </div>
            </div>
          </div>
...
<script>
  export default {
    ...
    methods: {
      getSubData(name,id, parentname, index,url,subUrl) {
